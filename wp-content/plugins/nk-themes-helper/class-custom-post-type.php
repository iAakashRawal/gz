...
class NKTH_Custom_Post_Type {
     ...
     * @param string|array $function Function to hook that will run on action.
    ...
    public function get_plural( $name = null ) {
        ...
        if ( strpos( $name, 'category' ) !== false ) {
            $name = str_replace( 'category', 'categorie', $name );
        }
    ...
    public function register_post_type() {
        ...
        $labels = array(
            ...
            'menu_name'          => $plural,
            'all_items'          => $plural,
            'add_new'            => __( 'Add New', 'nk-themes-helper' ),
            // translators: %s.
            'add_new_item'       => sprintf( __( 'Add New %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'edit_item'          => sprintf( __( 'Edit %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'new_item'           => sprintf( __( 'New %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'view_item'          => sprintf( __( 'View %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'search_items'       => sprintf( __( 'Search %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'not_found'          => sprintf( __( 'No %s found', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'parent_item_colon'  => sprintf( __( 'Parent %s:', 'nk-themes-helper' ), $singular ),
        );
    ...
    public function register_taxonomy( $taxonomy_names, $options = array() ) {
        ...
        $labels = array(
            ...
            'menu_name'                  => $plural,
            // translators: %s.
            'all_items'                  => sprintf( __( 'All %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'edit_item'                  => sprintf( __( 'Edit %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'view_item'                  => sprintf( __( 'View %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'update_item'                => sprintf( __( 'Update %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'add_new_item'               => sprintf( __( 'Add New %s', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'new_item_name'              => sprintf( __( 'New %s Name', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            'parent_item'                => sprintf( __( 'Parent %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'parent_item_colon'          => sprintf( __( 'Parent %s:', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'search_items'               => sprintf( __( 'Search %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'popular_items'              => sprintf( __( 'Popular %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'separate_items_with_commas' => sprintf( __( 'Seperate %s with commas', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'add_or_remove_items'        => sprintf( __( 'Add or remove %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'choose_from_most_used'      => sprintf( __( 'Choose from most used %s', 'nk-themes-helper' ), $plural ),
            // translators: %s.
            'not_found'                  => sprintf( __( 'No %s found', 'nk-themes-helper' ), $plural ),
        );
        ...
        switch ( $column ) {
            ...
            case ( taxonomy_exists( $column ) ):
                ...
                if ( ! empty( $terms ) ) {
                    ...
                    // Echo no terms.
                    // translators: %s.
                    printf( esc_html__( 'No %s', 'nk-themes-helper' ), esc_html( $taxonomy_object->labels->name ) );
                }
                break;
            ...
            if ( ! empty( $filters ) ) {
                ...
                foreach ( $filters as $tax_slug ) {
                    ...
                    if ( $terms ) {
                        ...
                        // Default show all.
                        // translators: %s - label.
                        printf( '<option value="0">%s</option>', sprintf( esc_html__( 'Show all %s', 'nk-themes-helper' ), esc_html( $tax->label ) ) );
                        ...
                        foreach ( $terms as $term ) {
                            ...
                            if ( isset( $_GET[ $tax_slug ] ) && $_GET[ $tax_slug ] === $term->slug ) {
                                printf( '<option value="%s" selected="selected">%s (%s)</option>', esc_attr( $term->slug ), esc_html( $term->name ), esc_html( $term->count ) );
    ...
    public function updated_messages( $messages ) {
        ...
        $messages[ $this->post_type_name ] = array(
            0  => '',
            // translators: %s.
            1  => sprintf( __( '%s updated.', 'nk-themes-helper' ), $singular ),
            2  => __( 'Custom field updated.', 'nk-themes-helper' ),
            3  => __( 'Custom field deleted.', 'nk-themes-helper' ),
            // translators: %s.
            4  => sprintf( __( '%s updated.', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            // phpcs:ignore
            5  => isset( $_GET['revision'] ) ? sprintf( __( '%2$s restored to revision from %1$s', 'nk-themes-helper' ), wp_post_revision_title( (int) $_GET['revision'], false ), $singular ) : false,
            // translators: %s.
            6  => sprintf( __( '%s updated.', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            7  => sprintf( __( '%s saved.', 'nk-themes-helper' ), $singular ),
            // translators: %s.
            8  => sprintf( __( '%s submitted.', 'nk-themes-helper' ), $singular ),
            9  => sprintf(
                // translators: %s.
                __( '%2$s scheduled for: <strong>%1$s</strong>.', 'nk-themes-helper' ),
                date_i18n( __( 'M j, Y @ G:i', 'nk-themes-helper' ), strtotime( $post->post_date ) ),
                $singular
            ),
            // translators: %s.
            10 => sprintf( __( '%s draft updated.', 'nk-themes-helper' ), $singular ),
        );
        return $messages;
    }
     ...
     * @param array $bulk_counts an array of bulk updated counts.
     *
     * @return array
     */
...
/**
 * Fallback for Class NK_Helper_Custom_Post_Type
 */
// phpcs:ignore
class NK_Helper_Custom_Post_Type extends NKTH_Custom_Post_Type {
}
