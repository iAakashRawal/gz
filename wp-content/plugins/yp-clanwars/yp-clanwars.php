<?php
/**
 * Plugin Name: YP-ClanWars (changed WP-ClanWars)
 ...
 * Author: Andrej Mihajlov, nK
 * Version: 1.7.4
 *
 * Changes: _nK http://nkdev.info
 *   Removed countries from teams
 *   Added logo to teams
 *
...
define('WP_CLANWARS_VERSION', '1.7.4');
...
require_once (dirname(__FILE__) . '/wp-clanwars-next-match-widget.php');
 ...
 */
add_action('init', 'youplay_matches_register');
function youplay_matches_register() {
	$args = array(
		'labels' => array(
			'name'                => _x('Matches', 'post type general name',WP_CLANWARS_TEXTDOMAIN),
			'singular_name'       => _x('Match', 'post type singular name',WP_CLANWARS_TEXTDOMAIN),
			'add_new'             => _x('Add New', 'match item' ,WP_CLANWARS_TEXTDOMAIN),
			'add_new_item'        => __('Add New Match',WP_CLANWARS_TEXTDOMAIN),
			'edit_item'           => __('Edit Match',WP_CLANWARS_TEXTDOMAIN),
			'new_item'            => __('New Match',WP_CLANWARS_TEXTDOMAIN),
			'view_item'           => __('View Match',WP_CLANWARS_TEXTDOMAIN),
			'search_items'        => __('Search Matches',WP_CLANWARS_TEXTDOMAIN),
			'not_found'           => __('Nothing found',WP_CLANWARS_TEXTDOMAIN),
			'not_found_in_trash'  => __('Nothing found in Trash',WP_CLANWARS_TEXTDOMAIN),
			'parent_item_colon'   => ''
		),
		'public'              => true,
...
add_action( 'admin_menu', 'youplay_remove_matches_menu' );
function youplay_remove_matches_menu() {
	remove_menu_page( 'edit.php?post_type=match' );
}

// Remove Featured Image Metabox from Custom Post Type Edit Screens
add_action('do_meta_boxes', 'youplay_remove_match_featured');
function youplay_remove_match_featured() {
	remove_meta_box('postimagediv', 'match', 'side');
}
...
class WP_ClanWars {
	...
	{
		// check if ACL exist and prevent add tables
		$acl = get_option(WP_CLANWARS_ACL, null);
		if($acl !== null) {
			return;
		}
	...
	function on_init()
	{
        ...
        call_user_func('add_' . 'shortcode', 'wp-clanwars', array($this, 'on_shortcode'));
		/* Add VC Shortcode */
	    if(function_exists("vc_map")) {
	        /* Register shortcode with Visual Composer */
	        vc_map( array(
	           "name"     => __("nK ClanWars", WP_CLANWARS_TEXTDOMAIN),
	           "base"     => "wp-clanwars",
	           "controls" => "full",
	           "category" => "nK",
	           "icon"     => "icon-nk icon-nk-clan-wars",
	           "params"   => array(
	              array(
	                 "type"        => "textfield",
	                 "heading"     => __("Matches count per page", WP_CLANWARS_TEXTDOMAIN),
	                 "param_name"  => "per_page",
	                 "value"       => __("6", WP_CLANWARS_TEXTDOMAIN),
	                 "description" => "",
	              ),
	           )
	        ) );
...
EOT;
	...
	function on_admin_menu()
	{
		...
		$menu_position = $this->is_jumpstarter() ? 3 : '59.11111';
	...
	function on_admin_post_setupteam() {
		if(!current_user_can('manage_options')) {
			wp_die(__('Cheatin&#8217; uh?', WP_CLANWARS_TEXTDOMAIN));
		}
	...
	{
		register_widget('WP_ClanWarsNextMatch_Widget');
	...
	function team_editor($page_title, $page_action, $page_submit, $team_id = 0)
	{
		...
		$context = \WP_Clanwars\Utils::extract_args(stripslashes_deep($_POST), \WP_Clanwars\Utils::extract_args($data, $defaults));
		$context['attach'] = isset($data['logo']) ? wp_get_attachment_image($data['logo'], 'thumbnail') : '';
	...
	function on_manage_teams()
	{
		...
		$page_links = paginate_links( array(
				...
				'format' => '',
				'prev_text' => __('&laquo;', WP_CLANWARS_TEXTDOMAIN),
				'next_text' => __('&raquo;', WP_CLANWARS_TEXTDOMAIN),
				'total' => $stat['total_pages'],
				'current' => $current_page
		));

		$page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s', WP_CLANWARS_TEXTDOMAIN ) . '</span>%s',
	...
	function on_match_shortcode($match_id) {
		...
		if(empty($matches)) {
			return sprintf('<p>' . esc_html__('Match with id = %s has been removed.', WP_CLANWARS_TEXTDOMAIN) . '</p>', $match_id);
		}
		...
		$context = compact('match', 'rounds', 'match_status_text', 'team1_logo', 'team2_logo');
	...
	function on_browser_shortcode($atts) {
		...
		$page_links = paginate_links(array(
			'prev_text' => __('&larr;', WP_CLANWARS_TEXTDOMAIN),
			'next_text' => __('&rarr;', WP_CLANWARS_TEXTDOMAIN),
			'total' => $stat['total_pages'],
			'current' => $current_page,
			'type' => 'array'
		));
		...
		for($i = 0; $i < sizeof($games); $i++) :
			...
			$output .= '[yp_tab ' . ($game->id == $current_game ? ' active="true"' : '') . ' title="' . $game->abbr . '"]';
				...
				foreach($matches as $index => $match) {
					...
					$output .= '<a class="youplay-single-match" href="' . get_permalink($match->post_id) . '" title="' . esc_attr($match->title) . '">';
						...
						$output .= '<div class="pull-left">
										...
										<h5 class="text-center">' . esc_html($match->team1_title) . '</h5>
									</div>';

						$output .= '<div class="pull-left ml-30">
										<div class="angled-img">
											<div class="img">
												<img src="' . esc_attr($team2_logo[0]) . '" alt="' . esc_html($match->team2_title) . '">
											</div>
										</div>
										<h5 class="text-center">' . esc_html($match->team2_title) . '</h5>
									</div>';
		...
		if(is_array($page_links)) {
			$output .= '<ul class="pagination dib">';

			foreach($page_links as $cur) {
				$output .= '<li ' . (strpos($cur, 'current') !== false ? 'class="active"' : '') . '>';
				$output .= wp_kses_post($cur);
				$output .= '</li>';
			}
	...
	function on_settings() {
		...
		$categories_dropdown = wp_dropdown_categories(array(
			'name' => 'category',
			'hierarchical' => true,
			'show_option_none' => __('None', WP_CLANWARS_TEXTDOMAIN),
			'hide_empty' => 0,
			'hide_if_empty' => 0,
